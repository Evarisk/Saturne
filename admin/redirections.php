...
$redirectionManager = new SaturneRedirection(DOL_DOCUMENT_ROOT . '/../.htaccess', $conf->global->MAIN_URL_ROOT);
...
if ($action == 'add') {
    try {
        $redirectionManager->addRedirection($fromUrl, $toUrl);
        setEventMessages($langs->trans('RedirectionAdded'), null);
    } catch (Exception $e) {
...
if ($action == 'remove') {
    try {
        $redirectionManager->removeRedirection($fromUrl);
        setEventMessages($langs->trans('RedirectionRemoved'), null);
    } catch (Exception $e) {
        setEventMessages($e->getMessage(), null, 'errors');
    }
    header('Location: ' . $_SERVER['PHP_SELF'] . '?module_name=' . $moduleName);
...
$redirections = $redirectionManager->listRedirections();
...
if (!empty($redirections)) {
    foreach ($redirections as $redirection) {
        // Generate QR Code
        $barcodeObject = new TCPDF2DBarcode($redirection['from'], 'QRCODE,H');
        ...
        print '</td><td>';
        print $redirection['to'];
        print '</td><td class="center">';
        print '<img src="' . $qrCodeBase64 . '" alt="QR Code" width="100" height="100">';
        print '</td><td class="center">';
        print '<input type="hidden" name="from_url" value="' . $redirection['from'] . '">';
...
print '<tr class="oddeven"><td>';
print '<input type="text" name="from_url" value="' . $fromUrl . '">';
print '</td><td>';
print '<input type="text" name="to_url" value="' . $toUrl . '">';
print '</td><td class="center">';
print '</td><td class="center">';
