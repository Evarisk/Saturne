 ...
 * Variables  : $arrayfields, $fieldsToSearchAll, $formMoreParams (optional), $helpText (optional) $nbTotalOfRecords, $num, $permissiontoadd, $resql, $search, $search_array_options (extrafields_list_search_param.tpl), $searchCategories, $sql, $title
 */
...
$newCardButton .= dolGetButtonTitle($langs->trans('New' . ucfirst($object->element)), $helpText ?? '', 'fa fa-plus-circle', dol_buildpath('custom/' . $object->module . '/view/' . $object->element . '/' . $object->element . '_card.php', 1) . '?action=create' . ($moreUrlParameters ?? ''), '', $permissiontoadd);
