...
function certificate_prepare_head($object)
{
	...
	$head[$h][0] = dol_buildpath("/saturne/view/certificate/certificate_card.php", 1).'?id='.$object->id;
	...
	if (isset($object->fields['note_public']) || isset($object->fields['note_private'])) {
		...
		$head[$h][0] = dol_buildpath('/saturne/view/certificate/certificate_note.php', 1).'?id='.$object->id;
	...
	$head[$h][0] = dol_buildpath("/saturne/view/certificate/certificate_agenda.php", 1).'?id='.$object->id;
	$head[$h][1] = '<i class="fas fa-calendar-alt pictofixedwidth"></i>' . $langs->trans("Events");
	$head[$h][2] = 'agenda';
	$h++;

	$head[$h][0] = dol_buildpath("/saturne/view/certificate/certificate_attendants.php", 1) . '?id=' . $object->id;
	$head[$h][1] = '<i class="fas fa-file-signature pictofixedwidth"></i>' . $langs->trans("Attendants");
	$head[$h][2] = 'attendants';
	$h++;

	complete_head_from_modules($conf, $langs, $object, $head, $h, 'certificate@saturne');
