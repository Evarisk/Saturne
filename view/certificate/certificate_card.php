...
require_once __DIR__ . '/../../class/saturnedocuments/certificatedocument.class.php';
...
$permissiontoread   = $user->rights->saturne->certificate->read;
$permissiontoadd    = $user->rights->saturne->certificate->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
$permissiontodelete = $user->rights->saturne->certificate->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
$permissionnote     = $user->rights->saturne->certificate->write; // Used by the include of actions_setnotes.inc.php

$upload_dir = $conf->saturne->multidir_output[isset($object->entity) ? $object->entity : 1];
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/saturne/view/certificate/certificate_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/saturne/view/certificate/certificate_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	$triggersendname = 'SATURNE_CERTIFICATE_SENTBYMAIL';
...
//	}
	...
	if ($action != 'presend') {
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-list-alt imgforviewmode', dol_buildpath('/saturne/view/certificate/certificate_agenda.php', 1).'?id='.$object->id);
	...
	$diroutput = $conf->saturne->dir_output;
	$trackid = 'certificate'.$object->id;
