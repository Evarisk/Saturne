...
require_once __DIR__ . '/../../lib/saturne_certificate.lib.php';
require_once __DIR__ . '/../../lib/saturne_function.lib.php';

global $db, $hookmanager, $langs, $user;

// Load translation files required by the page
$langs->loadLangs(array("saturne@saturne", "other"));
...
$permissiontoread   = $user->rights->saturne->$object_type->read;
$permissiontoadd    = $user->rights->saturne->$object_type->write;
$permissiontodelete = $user->rights->saturne->$object_type->delete;
...
if (empty($backtopage) || ($cancel && empty($id))) {
	if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
		$backtopage = dol_buildpath('/saturne/view/'. $object->element .'/' . $object->element .'_attendants.php', 1) . '?id=' . ($object->id > 0 ? $object->id : '__ID__');
	}
}
...
if ($action == 'addSocietyAttendant') {
	...
	if ( ! $error) {
		...
		if ($result > 0) {
			...
			$signatory->call_trigger('SATURNESIGNATURE_ADDATTENDANT', $user);
...
if ($action == 'send') {
	...
	if ( ! $error) {
		...
		if (dol_strlen($sendto) && ( ! empty($conf->global->MAIN_MAIL_EMAIL_FROM))) {
			...
			$url  = dol_buildpath('/custom/saturne/public/signature/add_signature.php?track_id=' . $signatory->signature_url  . '&type=' . $object->element, 3);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    $object->picto = 'certificate_small@saturne';
	...
	if ($signatory->checkSignatoriesSignatures($object->id, $object->element) && $object->status < $object::STATUS_LOCKED) {
		print '<a class="butAction" style="width = 100%;margin-right:0" href="' . DOL_URL_ROOT . '/custom/saturne/view/' . $object->element . '/' . $object->element . '_card.php?id=' . $id . '">' . $langs->trans("GoToLock") . '</a>';
	}
	...
	if (is_array($society_intervenants) && ! empty($society_intervenants) && $society_intervenants > 0) {
		foreach ($society_intervenants as $element) {
			...
			if ($object->status == $object::STATUS_VALIDATED) {
				$signatureUrl = dol_buildpath('/custom/saturne/public/signature/add_signature.php?track_id=' . $element->signature_url  . '&type=' . $object->element, 3);
			...
			if ($permissiontoadd && $object->status < $object::STATUS_LOCKED) {
				require __DIR__ . "/../../core/tpl/signature/saturne_signature_action_view.tpl.php";
			}
			print '</td>';
			print '<td class="center">';
			if ($element->signature != $langs->transnoentities("FileGenerated") && $permissiontoadd) {
				require __DIR__ . "/../../core/tpl/signature/saturne_signature_view.tpl.php";
			}
			print '</td>';
			print '</tr>';
	...
	if ($object->status == $object::STATUS_DRAFT && $conf->global->SATURNE_TIMESHEET_ADD_ATTENDANTS && $permissiontoadd) {
