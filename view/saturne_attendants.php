...
if ($id > 0 || !empty($ref) && empty($action)) {
    ...
    if ($reshook > 0) {
        $attendantsRole = $hookmanager->resArray;
    } else {
    ...
    if ($attendantTableMode == 'advanced') {
            ...
            if (is_array($signatories) && !empty($signatories) && $signatories > 0) {
                ...
                if ($object->status == $object::STATUS_DRAFT && $permissiontoadd) {
                    ...
                    if (!empty($selectedCompany) && $selectedCompany > 0 && $user->rights->societe->creer) {
                        $newcardbutton = '<a href="' . DOL_URL_ROOT . '/contact/card.php?socid=' . $selectedCompany . '&action=create' . $moreparam . urlencode('&newcompany' . $attendantRole . '=' . GETPOST('newcompany' . $attendantRole) . '&contactID=&#95;&#95;ID&#95;&#95;') . '" title="' . $langs->trans('NewContact') . '"><span class="fa fa-plus-circle valignmiddle paddingleft"></span></a>';
                        print $newcardbutton;
                    }
                    print '</td><td class="center">';
                    print '-';
        ...
        if (is_array($signatories) && !empty($signatories) && $signatories > 0) {
            ...
            if ($object->status == $object::STATUS_DRAFT && $permissiontoadd) {
                ...
                print '<td class=minwidth400">';
                if ($selectedCompany <= 0) {
                    print img_picto('', 'user', 'class="pictofixedwidth"') . $form->select_dolusers('', 'attendant_user', 1, $alreadyAddedUsers, 0, '', '', $conf->entity, 0, 0, '', 0, '', 'minwidth200 widthcentpercentminusx maxwidth300') . '<br>';
                }
                print img_object('', 'contact', 'class="pictofixedwidth"') . $form->selectcontacts(($selectedCompany > 0 ? $selectedCompany : -1), GETPOST('contactID'), 'attendant_contact', 1, $alreadyAddedUsers, '', 1, 'minwidth200 widthcentpercentminusx maxwidth300');
                if (!empty($selectedCompany) && $selectedCompany > 0 && $user->rights->societe->creer) {
                    $newcardbutton = '<a href="' . DOL_URL_ROOT . '/contact/card.php?socid=' . $selectedCompany . '&action=createle_mode=' . $moreparam . urlencode('&newcompany=' . GETPOST('newcompany') . '&contactID=&#95;&#95;ID&#95;&#95;') . '" title="' . $langs->trans('NewContact') . '"><span class="fa fa-plus-circle valignmiddle paddingleft"></span></a>';
                    print $newcardbutton;
                }
                print '</td><td class="center">';
            ...
            if ($object->status == $object::STATUS_DRAFT && $permissiontoadd) {
                ...
                print img_object('', 'contact', 'class="pictofixedwidth"') . $form->selectcontacts(($selectedCompany > 0 ? $selectedCompany : -1), GETPOST('contactID'), 'attendant_contact', 1, $alreadyAddedUsers, '', 1, 'minwidth200 widthcentpercentminusx maxwidth300');
                if (!empty($selectedCompany) && $selectedCompany > 0 && $user->rights->societe->creer) {
                    $newcardbutton = '<a href="' . DOL_URL_ROOT . '/contact/card.php?socid=' . $selectedCompany . '&action=create' . $moreparam . urlencode('&newcompany=' . GETPOST('newcompany') . '&contactID=&#95;&#95;ID&#95;&#95;') . '" title="' . $langs->trans('NewContact') . '"><span class="fa fa-plus-circle valignmiddle paddingleft"></span></a>';
                    print $newcardbutton;
                }
                print '</td><td class="center">';
                print saturne_select_dictionary('attendant_role','c_' . $object->element . '_attendants_role', 'ref');
