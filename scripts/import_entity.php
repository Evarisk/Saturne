 ...
 * The same import runs from the entity transfer admin page, both call
 * saturne_entity_transfer_import().
 */
...
if (!empty($manifest)) {
    ...
    print '  Modules       : ' . (empty($manifest['modules']) ? '-' : implode(', ', $manifest['modules'])) . "\n";
...
if (!isModEnabled('saturne')) {
    print "\nError: the Saturne module is not enabled on this install. Enable it first, so that\n";
...
if (!empty($missingModules)) {
    print "\nError: the dump holds the tables of " . implode(', ', $missingModules) . ", not enabled here.\n";
    print "Enable those modules first, so that their tables, rights, menus and document models are created.\n";
    exit(1);
}
...
$result = saturne_entity_transfer_import($db, $sqlFile, [
    'manifest'      => $manifest,
