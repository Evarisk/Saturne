 ...
 * The same export runs from the entity transfer admin page, both call
 * saturne_entity_transfer_export().
 */
...
if (isset($arguments['help']) || (!isset($arguments['entity']) && !isset($arguments['list-modules']))) {
    print "\n";
    ...
    print "  --scope=<scope>          module : Saturne tables and those of --with-modules (default)\n";
    ...
    print "  --with-modules=<list>    Also export the tables of those modules, comma separated.\n";
    print "                           Example: --with-modules=digiriskdolibarr,doliletter\n";
    print "  --with-dictionaries      Also export the dictionaries (llx_c_xxx) of Saturne and of those modules\n";
    ...
    print "                           fnmatch() patterns accepted, for a dictionary named after its\n";
    print "                           object rather than after its module: --include-table=c_lesion_*\n";
    print "  --exclude-table=<list>   Tables to skip, comma separated, without the llx_ prefix\n";
    print "  --output=<dir>           Output directory (default " . $conf->admin->dir_output . "/saturne_entity_export/...)\n";
    print "  --chunk=<n>              Rows per INSERT statement (default 200)\n";
    print "  --dry-run                Only print the plan and the number of rows\n";
    print "  --list-modules           Print the modules holding tables of their own, then exit\n";
...
$scope        = (string) ($arguments['scope'] ?? 'module');
...
if (!in_array($scope, ['module', 'core', 'full'], true)) {
    print "Error: --scope must be module, core or full.\n";
    exit(1);
}
...
$options = [
    ...
    'output_dir'        => rtrim(str_replace('\\', '/', (string) ($arguments['output'] ?? $defaultOutput)), '/')
];

$plan    = saturne_entity_transfer_build_plan($db, $options);
$counted = saturne_entity_transfer_count_plan($db, $plan, $sourceEntities);

print "Saturne entity export\n";
...
print '  Scope         : ' . $scope . "\n";
print '  Modules       : ' . (empty($modules) ? '-' : implode(', ', $modules)) . "\n";
...
$export = saturne_entity_transfer_export($db, $options);
